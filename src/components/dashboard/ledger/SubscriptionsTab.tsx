...
import { CreditCard, Zap, RefreshCw, Loader2, CalendarClock, Receipt, Download } from 'lucide-react';
...
export default function SubscriptionsTab() {
  ...
  const fetchAllData = async () => {
    ...
    // 1. FETCH SUBSCRIPTIONS (MRR & NEXT BILL)
    ...
    if (!error && activeStores) {
      setStorefronts(activeStores);
    }
    setIsLoading(false);

    // 2. FETCH ALL KNOWN STOREFRONTS FOR RECONCILIATION
    // We fetch this to cross-reference Stripe against our actual database
    ...
    if (invoiceData.success && invoiceData.invoices) {
      ...
      // 🚀 THE RECONCILER: 
      // Filter out ghost Stripe data (like deleted BarTok test accounts) 
      // by making sure the Stripe invoice matches a real Supabase storefront.
      const knownNames = allStores?.map(s => s.business_name?.toLowerCase()).filter(Boolean) || [];
      const knownEmails = allStores?.map(s => s.contact_email?.toLowerCase()).filter(Boolean) || [];
      
      const reconciledInvoices = invoiceData.invoices.filter((inv: any) => {
        const cName = inv.customerName?.toLowerCase() || '';
        const cEmail = inv.customerEmail?.toLowerCase() || '';
        
        // Keep the invoice if the Stripe email OR Stripe name matches our Supabase records
        const matchesEmail = knownEmails.includes(cEmail);
        const matchesName = knownNames.some(name => cName.includes(name) || name.includes(cName));
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-bg-surface-200/50 border border-emerald-500/30 rounded-2xl p-6 relative overflow-hidden shadow-[0_0_40px_rgba(16,185,129,0.05)]">
        ...
        <button 
          ...
          className="flex items-center justify-center gap-2 px-4 py-2 bg-white/5 text-slate-400 border border-white/10 rounded-lg text-xs font-mono uppercase tracking-widest transition-all hover:bg-white/10 hover:text-white shrink-0"
        >
