...
import { Send, Copy, AlertTriangle, CheckCircle2, Loader2, Mail, Clock, ShieldAlert, User } from 'lucide-react';
import { dispatchStagingReview } from '@/app/actions/storefronts';

export default function StagingDispatcher({ formData, setFormData }: { formData: any, setFormData: any }) {
  ...
  const logs = formData.staging_logs || [];
  ...
  const handleSendForReview = async () => {
    ...
    try {
      // 1. Build the new log entry
      const newLog = {
        id: Date.now().toString(),
        timestamp: new Date().toISOString(),
        action: 'REVIEW DISPATCHED',
        message: `Interactive staging link transmitted to ${targetEmail}`,
        actor: 'System Admin'
      };
      ...
      // 2. Force a hard save with the new status and logs
      ...
      // 4. Fire the staging workflow dispatcher
    ...
    <div className="bg-zinc-900/60 border border-zinc-800 rounded-3xl p-8 shadow-xl flex flex-col h-full space-y-8">
      ...
      <div className="flex items-center gap-4 border-b border-zinc-800/80 pb-6 shrink-0">
        ...
        <div>
          ...
          <p className="text-zinc-400 text-sm font-light mt-1">
            Dispatch the interactive review layer to the client for final sign-off.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-6 shrink-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-4 flex flex-col justify-center">
            <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest flex items-center gap-2 mb-1.5">
              <Mail size={12} className="text-cyan-400" /> Target Routing
            </span>
            {targetEmail ? (
              <span className="text-white font-bold tracking-wider">{targetEmail}</span>
            ) : (
              <span className="text-rose-400 font-bold tracking-wider flex items-center gap-2">
                <AlertTriangle size={14} /> Missing Email
              </span>
            )}
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <button 
            ...
            className="flex-1 flex items-center justify-center gap-3 bg-fuchsia-600 hover:bg-fuchsia-500 text-white px-8 py-4 rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-[0_0_20px_rgba(192,38,213,0.3)] disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
          >
          ...
          <button 
            ...
            className="flex items-center justify-center gap-2 bg-zinc-950 hover:bg-zinc-900 border border-zinc-700 hover:border-cyan-500 text-zinc-300 hover:text-cyan-400 px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all cursor-pointer"
          >
      ...
      <div className="flex-1 flex flex-col border-t border-zinc-800/80 pt-6 mt-2 overflow-hidden">
        <h3 className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] mb-4 flex items-center gap-2 shrink-0">
          <ShieldAlert size={12} className="text-zinc-400" /> Transmission Logs
        </h3>
              ...
              <div key={log.id} className="bg-zinc-950 border border-zinc-800/80 rounded-xl p-4 flex flex-col gap-2 shadow-sm">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded bg-fuchsia-500/10 border border-fuchsia-500/20 text-[9px] font-black text-fuchsia-400 uppercase tracking-widest">
                      {log.action}
                    </span>
                  </div>
                ...
                <div className="flex items-center gap-1.5 text-[9px] font-mono text-zinc-600 uppercase tracking-widest mt-1">
                  <User size={10} /> Executed by {log.actor}
                </div>
              </div>
            ))
          )}
