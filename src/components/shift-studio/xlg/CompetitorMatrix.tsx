  ...
  return (
    <div className="w-full pb-24 relative">
      ...
      <div className="relative w-full max-w-7xl mx-auto">
        ...
        <div className="hidden md:block absolute left-1/2 top-0 bottom-0 w-0.5 bg-linear-to-b from-transparent via-brand-primary/30 to-transparent -translate-x-1/2 z-0" />
              ...
              <div className="space-y-24">
                {section.ROWS.map((row: any, i: number) => (
                  <div key={i} className="relative flex flex-col md:flex-row items-center justify-center group">
                    ...
                    <div className="hidden md:flex absolute top-1/2 left-0 w-full h-0.5 z-0 -translate-y-1/2 opacity-60 group-hover:opacity-100 transition-opacity duration-500">
                      <div className="w-[45%] h-full bg-linear-to-l from-orange-500/50 to-transparent" />
                      <div className="w-[55%] h-full bg-linear-to-r from-brand-primary/60 to-transparent shadow-[0_0_15px_rgba(6,182,212,0.8)]" />
                    </div>

                    {/* LEFT NODE: THE OLD WAY */}
                    <div className="w-full md:w-4/12 md:pr-12 flex justify-end relative z-10 mb-8 md:mb-0">
                      {/* Fixed max-w-70 here! */}
                      <div className="max-w-70 w-full p-6 rounded-2xl bg-black/60 border border-white/5 backdrop-blur-md group-hover:border-orange-500/30 transition-all duration-500 shadow-lg group-hover:shadow-[0_0_30px_rgba(249,115,22,0.1)] group-hover:-translate-x-2">
                        <p className="text-white/50 font-light text-base leading-relaxed text-center md:text-right group-hover:text-orange-400/90 transition-colors duration-500">
                          {row.duct}
                        </p>
                      </div>
                    </div>
                    ...
                    {/* RIGHT NODE: THE VISUAL UPGRADE */}
                    <div className="w-full md:w-6/12 md:pl-12 flex justify-start relative z-10">
                      <div className="w-full max-w-120 rounded-2xl bg-brand-primary/5 border border-white/5 backdrop-blur-md group-hover:bg-brand-primary/10 group-hover:border-brand-primary/40 transition-all duration-500 shadow-lg group-hover:shadow-[0_0_30px_rgba(6,182,212,0.2)] overflow-hidden group-hover:translate-x-2 flex flex-col">
                        
                        {row.image && (
                          <div className="w-full h-32 md:h-48 relative border-b border-white/10 overflow-hidden bg-black/50">
                            <img 
                               src={row.image} 
                               alt={row.feature} 
                               className="absolute inset-0 w-full h-full object-cover object-top-left opacity-60 group-hover:opacity-100 group-hover:scale-105 transition-all duration-700" 
                            />
                        ...
                        <div className="p-6">
                           <p className="text-white/90 font-medium text-base leading-relaxed text-center md:text-left drop-shadow-[0_0_8px_rgba(255,255,255,0.1)] group-hover:drop-shadow-[0_0_10px_rgba(255,255,255,0.4)] transition-all duration-500">
                             {row.shift}
                           </p>
                        </div>
                      </div>
