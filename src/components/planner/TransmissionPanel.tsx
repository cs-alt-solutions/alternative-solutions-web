...
import { MessageCircle, Mic, Share2, Loader2, Image as ImageIcon, Radio } from 'lucide-react';
  ...
  async function handlePaste(e: React.ClipboardEvent<HTMLTextAreaElement>) {
    ...
    if (imageItem) {
      ...
      if (result.success && result.url) {
        ...
        console.error(`CRITICAL: Intercept failed: ${result.message || 'Unknown Error'}`);
      }
  ...
  return (
    <section className="h-full flex flex-col">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          ...
          {/* Dynamic awareness of where these logs are going */}
        ...
        <div className="bg-black/30 border border-white/5 rounded-xl p-6 flex-1 flex flex-col relative overflow-hidden">
           ...
           <div className="pt-4 border-t border-white/5 flex items-center justify-end gap-3 relative z-0">
              <button disabled className="flex items-center gap-2 px-4 py-2 rounded bg-white/5 text-white/30 text-[10px] font-mono uppercase tracking-widest border border-white/5 cursor-not-allowed">
