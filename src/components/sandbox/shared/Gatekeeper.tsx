...
interface GatekeeperProps {
  onUnlock: () => void;
  appTitle: string;
  pin: string; // Keeping this so we don't break the parent, we'll use it later for employee routing!
    ...
    <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center p-6 text-zinc-100 relative overflow-hidden">
      ...
      <div className="z-10 w-full max-w-sm flex flex-col items-center animate-in fade-in zoom-in-95 duration-500">
        ...
        <div className="bg-cyan-500/10 border border-cyan-500/30 px-4 py-1.5 rounded-full mb-12">
          <p className="text-cyan-400 text-[10px] font-black tracking-[0.2em] uppercase">
            {lockedMessage}
          </p>
        </div>

        {/* The Fast-Pass Button */}
        <button 
          onClick={onUnlock}
          className="w-full bg-cyan-500 hover:bg-cyan-400 text-zinc-950 font-black uppercase tracking-widest py-5 rounded-2xl shadow-[0_0_20px_rgba(34,211,238,0.3)] hover:shadow-[0_0_30px_rgba(34,211,238,0.5)] transition-all flex items-center justify-center gap-3 group"
        >
          Enter Workspace 
          <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
        </button>
        
        <p className="mt-8 text-[10px] font-mono text-zinc-600 uppercase tracking-widest flex items-center gap-2">
