/* src/components/sandbox/apps/logistics/LogisticsModals.tsx */
'use client';

import React, { useState } from 'react';
// Rule #3: Single Source of Truth - Verified imports for all utilized icons
import { Edit2, X, AlertTriangle, ArrowRightLeft, CheckCircle, Copy, ClipboardCheck, Send, Ban, Plus, Minus } from 'lucide-react';

// ==========================================
// 1. EDIT ITEM MODAL
// ==========================================
export const EditItemModal = ({ item, onSave, onClose, enrichedLocations, vendorList }: any) => {
  ...
  const toggleLocation = (locId: string) => {
    setEditedItem((prev: any) => {
      ...
      if (isSelected) {
        return { ...prev, locations: prev.locations.filter((id: string) => id !== locId) };
      } else {
  ...
  return (
    <div className="fixed inset-0 bg-zinc-950/80 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
      <div className="bg-zinc-900 border border-zinc-700 rounded-4xl p-6 w-full max-w-md shadow-[0_0_40px_rgba(0,0,0,0.8)] animate-in zoom-in-95 duration-200">
          ...
          <div className="pt-2">
            ...
            <div className="flex flex-wrap gap-2">
              {Object.values(enrichedLocations).map((loc: any) => {
                ...
                return (
                  <button type="button" key={loc.id} onClick={() => toggleLocation(loc.id)} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all border ${isSelected ? `${loc.theme.bgLight} ${loc.theme.border} ${loc.theme.text} shadow-inner` : 'bg-zinc-950 border-zinc-800 text-zinc-500 hover:text-zinc-300 hover:border-zinc-700'}`}>
                    {loc.name}
                  </button>
                );
              })}
...
// ==========================================
// 2. SHORTAGE ALLOCATOR MODAL
// ==========================================
export const ShortageAllocatorModal = ({ itemId, catalog, orders, onResolve, onCancel, enrichedLocations }: any) => {
  ...
  return (
    <div className="fixed inset-0 bg-zinc-950/80 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
      <div className="bg-zinc-900 border border-zinc-700 rounded-4xl p-6 w-full max-w-sm shadow-[0_0_40px_rgba(0,0,0,0.8)] animate-in zoom-in-95 duration-200">
        ...
        <div className="space-y-3 mb-6">
          {locationsWithItem.map(locId => {
            const locInfo = Object.values(enrichedLocations).find((l: any) => l.id === locId) as any;
            const currentQty = orders[locId][itemId];
            const Icon = locInfo.icon;
            const theme = locInfo.theme;

            return (
              <button key={locId} onClick={() => onResolve(locId, itemId)} className={`w-full flex items-center justify-between p-4 rounded-xl border border-zinc-700 bg-zinc-800 hover:border-rose-500 hover:bg-rose-950/30 transition-all group text-left`}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-black text-lg bg-zinc-900 border border-zinc-700 px-3 py-0.5 rounded-lg text-zinc-300 group-hover:text-rose-400">{currentQty}</span>
                </div>
              </button>
            );
          })}
...
// ==========================================
// 3. ITEM REALLOCATION MODAL
// ==========================================
export const ItemReallocationModal = ({ itemId, catalog, orders, onUpdateQuantity, onClose, enrichedLocations }: any) => {
  const itemData = catalog.find((i: any) => i.id === itemId);
    ...
    <div className="fixed inset-0 bg-zinc-950/80 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
      <div className="bg-zinc-900 border border-zinc-700 rounded-4xl p-6 w-full max-w-md shadow-[0_0_40px_rgba(0,0,0,0.8)] animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={onClose} disabled={unallocated > 0} className={`p-2 rounded-full transition-colors ${unallocated > 0 ? 'bg-zinc-800 text-zinc-600 cursor-not-allowed' : 'bg-zinc-800 border border-zinc-700 text-zinc-400 hover:text-zinc-100'}`}>
            <X size={20} />
          </button>
        </div>
        ...
        <div className="mb-6 flex flex-col gap-3 bg-fuchsia-900/10 p-4 rounded-xl border border-fuchsia-500/20">
           <div className="flex items-center justify-between">
             <div>
               <p className="font-bold text-fuchsia-100 text-lg">{itemData.name}</p>
             ...
             <div className="text-right">
               ...
               <p className="font-black text-4xl text-fuchsia-400 leading-none drop-shadow-[0_0_8px_rgba(232,121,249,0.6)]">{maxTotal}</p>
             </div>
           </div>
            ...
            return (
              <div key={loc.id} className="flex items-center justify-between p-3 rounded-xl border border-zinc-800 bg-zinc-800 shadow-md">
                ...
                <div className="flex items-center gap-1 bg-zinc-900 rounded-lg p-1 border border-zinc-700 shadow-inner">
                  <button onClick={() => onUpdateQuantity(loc.id, itemId, -1)} disabled={currentQty === 0} className={`p-1.5 rounded-md transition-colors ${currentQty > 0 ? `text-zinc-400 hover:bg-zinc-800 hover:text-rose-400 border border-transparent hover:border-rose-500/30` : 'text-zinc-700'}`}>
                    <Minus size={16} />
                  </button>
                  <span className={`w-8 text-center font-black text-lg ${currentQty > 0 ? theme.text : 'text-zinc-100'}`}>
                    {currentQty}
                  </span>
                  <button onClick={() => onUpdateQuantity(loc.id, itemId, 1)} disabled={unallocated === 0} className={`p-1.5 rounded-md transition-colors ${unallocated > 0 ? `text-zinc-400 hover:bg-zinc-800 hover:${theme.text} border border-transparent hover:${theme.border}` : 'text-zinc-700 opacity-50 cursor-not-allowed'}`}>
                    <Plus size={16} />
                  </button>
                </div>
              </div>
        ...
        <button onClick={onClose} disabled={unallocated > 0} className={`w-full py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-md ${unallocated > 0 ? 'bg-zinc-800 border border-zinc-700 text-zinc-500 cursor-not-allowed' : 'text-zinc-950 bg-cyan-500 hover:bg-cyan-400 shadow-[0_0_15px_rgba(34,211,238,0.4)]'}`}>
          {unallocated > 0 ? (
...
// ==========================================
// 4. SHARE MANIFEST MODAL
// ==========================================
export const ShareManifestModal = ({ manifestType, orders, catalog, purchasedFrom, deliveredStatus, oosStatus, masterShoppingList, deliveryStartTime, onClose, enrichedLocations, vendorList }: any) => {
  ...
  const generateManifestText = () => {
    ...
    if (manifestType === 'grocery') {
      ...
      [...vendorList, "Unassigned"].forEach((vendorName: string) => {
        ...
        if (vendorItems.length > 0) {
          ...
          vendorItems.forEach(([itemId, data]: any) => {
            const itemData = catalog.find((i: any) => i.id === itemId);
      ...
      Object.values(enrichedLocations).forEach((loc: any) => {
  ...
  return (
    <div className="fixed inset-0 bg-zinc-950/80 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
      <div className={`bg-zinc-900 border ${isOosTheme ? 'border-rose-900/50 shadow-[0_0_40px_rgba(244,63,94,0.3)]' : 'border-zinc-700 shadow-[0_0_40px_rgba(0,0,0,0.8)]'} rounded-4xl p-6 w-full max-w-md animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]`}>
        ...
        <button onClick={handleCopy} className={`w-full py-4 rounded-xl font-black flex items-center justify-center gap-2 transition-all shadow-md ${copied ? (isOosTheme ? 'bg-rose-500 text-zinc-950 shadow-[0_0_15px_rgba(244,63,94,0.4)]' : 'bg-cyan-500 text-zinc-950 shadow-[0_0_15px_rgba(34,211,238,0.4)]') : 'bg-zinc-800 border border-zinc-700 text-zinc-100 hover:bg-zinc-700 hover:border-cyan-500/50'}`}>
