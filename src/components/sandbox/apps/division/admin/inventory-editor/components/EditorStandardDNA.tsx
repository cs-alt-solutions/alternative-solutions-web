...
import { Wind, Cookie, Droplet, Sparkles } from 'lucide-react';

export default function EditorStandardDNA({ updatedItem, setUpdatedItem, descMode, setDescMode, showDNA, showLineage }: any) {
  return (
    <div className="space-y-5 animate-in fade-in zoom-in-95 duration-300">
      <div className="bg-zinc-950/50 border border-zinc-800/80 rounded-2xl p-5 mb-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
          <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider flex items-center gap-2">Storytelling Format</label>
          <div className="flex bg-zinc-950 border border-zinc-800 rounded-lg p-1">
            <button onClick={(e) => { e.preventDefault(); setDescMode('desc'); }} className={`px-4 py-1.5 rounded-md text-[9px] font-black uppercase tracking-widest transition-colors ${descMode === 'desc' ? 'bg-zinc-800 text-zinc-100 shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}>
              Standard Description
            </button>
            <button onClick={(e) => { e.preventDefault(); setDescMode('fact'); }} className={`px-4 py-1.5 rounded-md text-[9px] font-black uppercase tracking-widest transition-colors flex items-center gap-1.5 ${descMode === 'fact' ? 'bg-pink-500/20 text-pink-400 border border-pink-500/30 shadow-sm' : 'text-zinc-500 hover:text-pink-400/50'}`}>
              <Sparkles size={10} /> Insider Fact
            </button>
          </div>
        </div>
        {descMode === 'desc' ? (
          <textarea value={updatedItem.descBase || ''} onChange={(e) => setUpdatedItem({ ...updatedItem, descBase: e.target.value })} rows={3} placeholder="Write a traditional product description..." className="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-3.5 text-sm font-medium text-zinc-300 focus:border-emerald-500/50 outline-none transition-colors resize-none" />
        ) : (
          <textarea value={updatedItem.descFact || ''} onChange={(e) => setUpdatedItem({ ...updatedItem, descFact: e.target.value })} rows={3} placeholder="Write a short, punchy insider fact..." className="w-full bg-zinc-900 border border-pink-500/30 rounded-xl p-3.5 text-sm font-medium text-pink-100 focus:border-pink-500 outline-none transition-colors resize-none" />
        )}
      </div>
