// sandbox/apps/admin/storefront/InventorySelectorModal.tsx
'use client';
      ...
      <div className="bg-zinc-950 border border-zinc-800 rounded-4xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col h-[80vh]">
        ...
        <div className="flex-1 overflow-y-auto p-4 scrollbar-hide space-y-2">
          {filteredItems.length > 0 ? filteredItems.map((item: any) => {
             ...
             // FIXED: Grabbing imageUrl from your actual database property
             const imageSrc = item.imageUrl || item.imgUrl || item.image;
             return (
               <button 
                 key={item.id} 
               ...
               >
                 <div className="flex items-center gap-4">
                   <div className="w-12 h-12 rounded-xl bg-zinc-950 overflow-hidden border border-zinc-800 flex items-center justify-center shrink-0">
                     {imageSrc ? <img src={imageSrc} className="w-full h-full object-cover" alt={item.name} /> : <Package size={16} className="text-zinc-700"/>}
                   </div>
                   <div>
