...
export const StorefrontCard = ({ item, cart, updateCart, isHero = false }: { item: any, cart: any, updateCart: (itemId: string, size: any, options: any[], delta: number) => void, isHero?: boolean }) => {
  ...
  // IRONCLAD SAFEGUARDS: Filter out any null/undefined ghost objects from the DB
  ...
  // CATEGORY ENFORCEMENT: Flower and Merch NEVER show variant options
  ...
  // Enforce correct stock reading based on category
  const maxStockForDisplay = forceHideVariants ? item?.onHand : (initialOption?.stock !== undefined ? initialOption.stock : item?.onHand);
  const isMaxReached = qty > 0 && qty >= maxStockForDisplay; 

  // SAFEGUARD: Use ?. to prevent crashes if a size object is corrupted
