  ...
  return (
    <div className="absolute inset-0 w-full h-full backface-hidden transform-[rotateY(180deg)] bg-zinc-950 border border-zinc-800 rounded-[2.5rem] p-5 flex flex-col shadow-2xl overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide min-h-20 pr-1">
        {(item?.descBase || hasDNA) ? (
          <div className="bg-zinc-900/50 border border-zinc-800/50 rounded-xl p-4 flex flex-col gap-3">
               ...
               <div className="grid grid-cols-3 gap-3">
                 {item?.descFeels && (
                   <div className="flex items-start gap-2 border-r border-zinc-800/50 pr-2">
                     ...
                     <div>
                       <span className="text-[9px] font-black uppercase tracking-widest text-zinc-500 block mb-1">Feels</span>
                 ...
                 {item?.descTaste && (
                   <div className="flex items-start gap-2 border-r border-zinc-800/50 px-2">
                     ...
                     <div>
                       <span className="text-[9px] font-black uppercase tracking-widest text-zinc-500 block mb-1">Taste</span>
                 ...
                 {item?.descUses && (
                   <div className="flex items-start gap-2 pl-2">
                     ...
                     <div>
                       <span className="text-[9px] font-black uppercase tracking-widest text-zinc-500 block mb-1">Uses</span>
             ...
             {item?.descFact && (
               <div className="flex items-start gap-2 border-t border-zinc-800/50 pt-3">
                 ...
                 <div>
                   <span className="text-[9px] font-black uppercase tracking-widest text-zinc-500 block mb-1">Insider Fact</span>
          ...
          <div className="h-full flex items-center justify-center">
            <span className="text-xs font-black uppercase tracking-widest text-zinc-600">No Product DNA Available</span>
          </div>
        )}
      </div>
      ...
      <div className="shrink-0 flex flex-col gap-3 pt-3 mt-3 border-t border-zinc-800/50">
        ...
        {sizes.length > 1 && (
          <div>
            <div className="flex flex-wrap items-center bg-zinc-900 border border-zinc-800/50 rounded-xl p-1.5 gap-1.5">
              {sizes.map((s: any) => {
                if (!s) return null;
                ...
                return (
                  <button 
                    ...
                    className={`flex-1 min-w-20 px-2 py-2 rounded-lg text-center transition-all ${isSelected ? 'bg-zinc-100 text-zinc-950 shadow-md' : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50'}`}
                  >
                    <span className="block text-[10px] font-black uppercase tracking-widest leading-none mb-1">{s.label}</span>
                    <span className={`flex text-xs md:text-sm font-mono font-bold leading-none justify-center items-center gap-1 ${isSelected ? 'text-emerald-700' : item?.dailyDeal ? 'text-pink-400' : 'text-emerald-400'}`}>
                      {tierFinalPrice < activePrice && <span className="line-through text-zinc-500/50 text-[10px]">${activePrice.toFixed(0)}</span>}
        ...
        {hasMultipleOptions && (
          <div>
            <div className="flex justify-between items-center mb-2 px-1">
              <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Select Options</label>
              {bundleQty > 1 && (
                <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${isBundleComplete ? 'bg-emerald-500/20 text-emerald-400' : 'bg-zinc-800 text-zinc-400'}`}>
        ...
        <div className="shrink-0 mt-2">
          {!isBundleComplete && bundleQty > 1 ? (
            ...
            <button 
              ...
              className="w-full bg-emerald-500 hover:bg-emerald-400 text-zinc-950 font-black uppercase tracking-widest py-4 rounded-xl transition-all shadow-[0_5px_20px_rgba(52,211,153,0.3)] active:scale-95 flex items-center justify-center gap-2 text-xs md:text-sm"
            >
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between bg-zinc-900 border border-emerald-500/30 p-2 rounded-xl">
                ...
                <div className="flex flex-col items-center justify-center px-4">
                  ...
                  <span className="text-[9px] font-black uppercase tracking-widest text-emerald-500 flex items-center gap-1 mt-1"><CheckCircle size={10}/> In Cart</span>
                </div>
                <button 
                  onClick={() => updateCart(item.id, selectedSize, selectedOptions, 1)}
                  disabled={isMaxReached}
                  className="w-12 h-10 flex items-center justify-center bg-zinc-950 hover:bg-zinc-800 rounded-lg text-emerald-400 transition-colors active:scale-90 border border-zinc-800 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <Plus size={16} />
                </button>
              </div>
              ...
              <div className="flex items-center justify-between px-2 py-1">
                 <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Live Subtotal</span>
              ...
              {savingsText && (
                 <div className="text-center bg-pink-500/10 border border-pink-500/20 py-2 rounded-lg animate-in zoom-in-95 mt-1">
