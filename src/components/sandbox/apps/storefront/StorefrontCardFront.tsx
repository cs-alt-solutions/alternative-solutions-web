...
export default function StorefrontCardFront({ item, cleanItemName, baseLowestPrice, activeLowestPrice, setIsFlipped, sizes, isFlower, clientConfig }: any) {
  ...
  if (typeStr.includes('sativa')) strainColorClass = "text-orange-400";
  else if (typeStr.includes('indica')) strainColorClass = "text-purple-400";
  else if (typeStr.includes('hybrid')) strainColorClass = "text-emerald-400";
  ...
  return (
    <div className={`absolute inset-0 w-full h-full backface-hidden bg-zinc-900/90 backdrop-blur-xl border border-zinc-800 rounded-[2.5rem] flex flex-col shadow-xl overflow-hidden transition-all duration-500 ${isOutOfStock ? 'opacity-85 grayscale-30' : ''}`}>
      ...
      <div className="relative w-full h-[45%] shrink-0 bg-zinc-950 border-b border-zinc-800 overflow-hidden group">
        ...
        {/* Anchored Top-Left: Promos & Features */}
        ...
        {/* Anchored Bottom-Right: Strain Indicator (Simplified from a bright pill to a sleek tag) */}
        {item?.strainType && item.strainType !== 'N/A' && (
           <div className="absolute bottom-3 right-3 z-10 flex items-center gap-1.5 bg-zinc-950/80 backdrop-blur-md px-2.5 py-1 rounded-lg border border-zinc-800/80 shadow-lg">
      ...
      <div className="flex flex-col flex-1 p-5 text-center">
          ...
          {/* Subtle Category/Brand Above Title */}
          <div className="mt-auto mb-1 flex items-center justify-center gap-2 text-[9px] font-black uppercase tracking-widest">
             {item?.subCategory && (
          ...
          <div className="mb-auto w-full flex flex-col items-center overflow-hidden">
             ...
             {/* Sleek Lineage Display (De-Pilled) */}
             {item?.lineage && (
