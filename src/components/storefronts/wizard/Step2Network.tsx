    ...
    <div className="animate-in fade-in slide-in-from-bottom-8 duration-500 space-y-12">
      <div className="space-y-4">
        <h1 className="text-4xl md:text-5xl font-black tracking-tight text-white capitalize">
          The <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400">Network.</span>
        </h1>
        <p className="text-lg text-zinc-400 leading-relaxed">
          Which platforms are you active on? Select a network and drop your link so I can wire them up.
        </p>
      </div>
      ...
      <div className="space-y-3">
        {['instagram', 'facebook', 'x', 'linkedin'].map((network) => (
          <div 
            key={network} 
            className={`flex flex-col sm:flex-row sm:items-center w-full bg-zinc-900/40 rounded-2xl border overflow-hidden transition-all duration-300 focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500 ${activeSocials[network] ? 'border-indigo-500/50 shadow-[0_0_15px_rgba(99,102,241,0.1)]' : 'border-zinc-800 hover:border-zinc-700'}`}
          >
            <button 
              type="button" 
              onClick={() => toggleSocial(network)} 
              className={`flex items-center justify-between p-5 transition-all ${activeSocials[network] ? 'w-full sm:w-48 bg-zinc-900/80 border-b sm:border-b-0 sm:border-r border-zinc-800' : 'w-full'}`}
            >
            ...
            {activeSocials[network] && (
              <div className="flex-1 flex items-center bg-zinc-950 p-2 sm:p-0 animate-in fade-in zoom-in-95 duration-200">
                 <input 
                  type="text"
                  placeholder="@username or URL"
                  value={socialHandles[network]}
                  onChange={(e) => handleSocialInputChange(network, e.target.value)}
                  className="w-full bg-transparent py-4 px-5 text-white focus:outline-none text-base"
                  autoFocus
                />
              </div>
            )}
        ...
        >
          <button 
            ...
            className={`flex items-center justify-between p-5 transition-all ${activeSocials['other'] ? 'w-full sm:w-48 bg-zinc-900/80 border-b sm:border-b-0 sm:border-r border-zinc-800' : 'w-full'}`}
          >
            <div className="flex items-center gap-3 text-zinc-300">
              <LinkIcon className="w-5 h-5" /> 
              <span className="font-medium text-lg text-white">Other Link</span>
            </div>
      ...
      <div className="bg-indigo-500/5 border border-indigo-500/20 rounded-2xl p-6 flex items-start gap-4">
          ...
          <div>
            <h4 className="font-bold text-white text-lg mb-1">How we communicate</h4>
            <p className="text-zinc-400 leading-relaxed text-sm">
              I will route all project updates and credential links straight to your email. This keeps everything in one searchable place.
            </p>
          </div>
      </div>
      ...
      <div className="flex gap-4 pt-4">
        ...
        <button 
          type="button" 
          onClick={onNext}
          className="relative flex-1 overflow-hidden py-5 rounded-2xl font-bold tracking-wide flex items-center justify-center gap-3 transition-all text-white border border-indigo-500/50 hover:border-indigo-400 hover:shadow-[0_0_20px_rgba(99,102,241,0.2)] hover:scale-[1.02]"
        >
          <div className="absolute left-0 top-0 h-full w-[50%] bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500 opacity-60 transition-all duration-700 ease-out"></div>
          <span className="relative z-10">Sweet. Bring on the visuals 🎨</span>
        </button>
      </div>
    </div>
  );
