...
import { ArrowLeft, CheckCircle2, ChevronDown, ChevronUp, Globe, Zap, Rocket } from 'lucide-react';
    ...
    <div className="animate-in fade-in slide-in-from-bottom-8 duration-500 space-y-12">
      ...
      <div className="bg-zinc-900/40 border border-zinc-800/50 p-6 md:p-8 rounded-3xl shadow-xl space-y-8">
        ...
        <div className="space-y-4">
          {plans.map((plan) => (
            <div 
              ...
              className={`border-2 rounded-2xl transition-all duration-300 overflow-hidden ${selectedPlan === plan.id ? 'bg-fuchsia-500/10 border-fuchsia-500 shadow-[0_0_20px_rgba(217,70,239,0.15)]' : 'bg-zinc-950 border-zinc-800 hover:border-zinc-700'}`}
            >
              <div className="p-6 flex items-center cursor-pointer">
                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mr-5 shrink-0 ${selectedPlan === plan.id ? 'border-fuchsia-500' : 'border-zinc-600'}`}>
              ...
              {expandedPlan === plan.id && (
                <div className="px-6 pb-6 pt-2 border-t border-zinc-800/50 ml-15">
        ...
        <div className="p-6 bg-zinc-950 border border-zinc-800 rounded-2xl">
          <label className="flex items-center gap-4 cursor-pointer group">
            <div className="relative flex items-center justify-center w-6 h-6 shrink-0">
              <input 
                ...
                className="appearance-none w-6 h-6 border-2 border-zinc-600 rounded bg-zinc-900 checked:bg-fuchsia-500 checked:border-fuchsia-500 cursor-pointer transition-colors" 
              />
          ...
          {wantsCustom && (
            <div className="mt-6 pl-10 border-l-2 border-zinc-800 ml-3 space-y-4">
              {selectedPlan === 'foundation' ? (
                <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                  <p className="text-sm text-zinc-400 mb-3">
                    Custom builds require a dedicated domain. Since you selected the <span className="text-white font-semibold">Foundation ($5)</span> tier, you must provide your existing professional domain URL to qualify. <span className="italic">(Note: Social media links like Facebook do not count).</span>
                  </p>
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-zinc-900 border border-zinc-700 rounded-xl py-4 pl-12 pr-4 text-white focus:outline-none focus:border-fuchsia-500 transition-all text-lg" 
                    />
                  </div>
                </div>
              ) : (
        ...
        {/* Priority Checkbox */}
        <div className={`p-6 rounded-2xl border-2 transition-all duration-300 ${priorityQueue ? 'bg-pink-500/10 border-pink-500/50 shadow-[0_0_20px_rgba(236,72,153,0.15)]' : 'bg-zinc-950 border-zinc-800 hover:border-zinc-700'}`}>
           <label className="flex items-start gap-4 cursor-pointer group">
             <div className="relative flex items-center justify-center w-6 h-6 shrink-0 mt-1">
               <input 
                 ...
                 className="appearance-none w-6 h-6 border-2 border-zinc-600 rounded bg-zinc-900 checked:bg-pink-500 checked:border-pink-500 cursor-pointer transition-colors" 
               />
             ...
             <div>
                ...
                <p className="text-sm text-zinc-400 leading-relaxed mt-2">
                  Because I hand-build these systems myself, there's often a waitlist. Checking this box adds a $1 retainer to your application. It proves you're serious and bumps you straight to the front of my queue for review.
                </p>
             </div>
           </label>
        </div>
