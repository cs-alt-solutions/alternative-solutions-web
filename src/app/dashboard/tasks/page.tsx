...
export default function StrategicPlannerPage() {
  ...
  useEffect(() => {
    async function fetchPlannerData() {
      ...
      setActiveDraft(draftData?.[0] || null);
      ...
      if (directiveData) {
        const unscheduled = directiveData
        ...
        setWeekFlow(prevWeek => prevWeek.map(dayObj => {
          const tasksForDay = directiveData
            .filter(task => {
    ...
    if (!isModalOpen) fetchPlannerData();
