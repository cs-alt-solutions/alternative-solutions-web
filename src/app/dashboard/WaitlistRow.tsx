...
import { MoreVertical, Mail, Key, UserCheck, RefreshCw, ExternalLink, MessageSquare } from 'lucide-react';
...
export default function WaitlistRow({ entry, isSelected, onToggleSelect }: WaitlistRowProps) {
  ...
  // Safely format the date depending on if it comes from dummy data or Supabase
  ...
  return (
    <tr className={`border-b border-white/5 transition-colors group relative ${isSelected ? 'bg-brand-primary/5' : 'hover:bg-white/2'}`}>
      ...
      {/* USER CELL: Name (if available) + Email */}
      <td className="px-6 py-4">
        ...
        <div className="text-[10px] font-mono text-white/50">{entry.email}</div>
      </td>

      {/* CONTACT CELL: Phone + SMS Badge */}
      <td className="px-6 py-4">
        {entry.phone ? (
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-mono text-white/80">{entry.phone}</span>
            {entry.sms_consent && (
              <span className="flex items-center gap-1 text-[8px] bg-brand-primary/10 text-brand-primary px-1.5 py-0.5 rounded border border-brand-primary/20 tracking-widest uppercase font-bold">
                <MessageSquare size={8} /> SMS OK
              </span>
            )}
          </div>
        ) : (
