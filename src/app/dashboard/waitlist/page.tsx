...
import { 
  ...
  AlertTriangle, CheckCircle2, Bot, PlusCircle, Bug, Inbox, FolderArchive, Map
} from 'lucide-react';
...
export default function BetaWaitlistCommand() {
  ...
  useEffect(() => {
    async function fetchWaitlist() {
      ...
      if (error) {
        ...
        // ARMOR PLATE: The .filter(Boolean) drops any null/undefined rows right here.
  ...
  const handleAuthorizeDeployment = () => {
    console.log(`Simulating Deployment to ${selectedIds.size} users.`);
    alert("Backend logic not yet connected. Blueprint layout approved.");
  ...
  return (
    <div className="min-h-screen bg-bg-app text-text-main p-8 relative overflow-hidden flex flex-col font-sans">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="bg-black/40 border border-white/10 rounded-lg p-3 w-64">
               <div className="flex justify-between items-center mb-1">
                 <span className="text-[10px] font-mono text-white/50 uppercase">Cohort Capacity</span>
               ...
               <>
                 <div className="p-4 border-b border-white/5 flex justify-between items-center bg-white/2">
                    <div className="flex gap-4">
                        ...
                        <div className="flex items-center gap-2 bg-black/50 border border-white/10 rounded-md px-3 py-1.5 focus-within:border-brand-primary/50 transition-colors">
                           <span className="text-[10px] font-mono text-white/40 uppercase mr-2">STATE:</span>
                           <select className="bg-transparent text-[10px] font-mono text-white uppercase focus:outline-none cursor-pointer">
                 ...
                 <div className="flex-1 overflow-auto relative min-h-100">
                    {isLoading ? (
               ...
               <div className="flex flex-col h-full bg-black/20">
                  ...
                  <div className="flex items-center gap-1 p-4 border-b border-white/5 bg-black/40">
                     <button onClick={() => setFeedbackFolder('INBOX')} className={`px-4 py-2 rounded-md text-[10px] font-mono uppercase tracking-widest flex items-center gap-2 transition-colors ${feedbackFolder === 'INBOX' ? 'bg-brand-primary text-black font-bold' : 'text-white/50 hover:bg-white/5 hover:text-white'}`}>
                       <Inbox size={14} /> {copy.FEEDBACK_VIEW.FOLDERS.INBOX} <span className="bg-black/20 px-1.5 rounded">{MOCK_TELEMETRY.length}</span>
                     </button>
                     ...
                     <button onClick={() => setFeedbackFolder('ROADMAP')} className={`px-4 py-2 rounded-md text-[10px] font-mono uppercase tracking-widest flex items-center gap-2 transition-colors ${feedbackFolder === 'ROADMAP' ? 'bg-blue-500 text-white font-bold' : 'text-white/50 hover:bg-white/5 hover:text-white'}`}>
                       <Map size={14} /> {copy.FEEDBACK_VIEW.FOLDERS.ROADMAP}
                     </button>
                  ...
                  <div className="flex-1 overflow-auto p-6 relative">
                    ...
                    {feedbackFolder !== 'INBOX' ? (
                      <div className="absolute inset-0 flex flex-col items-center justify-center opacity-30 select-none">
                         ...
                         <p className="text-sm font-bold uppercase tracking-widest font-mono">FOLDER EMPTY</p>
                         <p className="text-[10px] font-mono mt-2">No items currently stored in this view.</p>
                      </div>
                    ) : (
                      <>
                        <div className="mb-8 p-4 bg-brand-primary/5 border border-brand-primary/20 rounded-lg flex items-start gap-4">
                          <div className="p-2 bg-brand-primary/10 rounded-full shrink-0">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-white/70 font-mono leading-relaxed">
                              {copy.FEEDBACK_VIEW.AI_SUMMARY.DESC}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="grid gap-4">
                          {MOCK_TELEMETRY.map((log) => (
                            <div key={log.id} className="bg-bg-surface-100 border border-white/5 p-5 rounded-lg flex flex-col gap-6 hover:border-brand-primary/20 transition-colors">
                               ...
                               <div className="flex flex-col md:flex-row md:items-start gap-6">
                                 <div className="md:w-1/4">
                                   ...
                                   <div className={`inline-flex items-center gap-2 px-2.5 py-1.5 rounded border ${
                                     ...
                                     log.aiTag === 'FEATURE REQUEST' ? 'bg-blue-500/10 text-blue-400 border-blue-500/30' :
                                     'bg-white/5 text-white/50 border-white/10'
                                   }`}>
                                      <Bot size={12} />
                                      <span className="text-[10px] font-bold tracking-widest uppercase">{log.aiTag}</span>
                                      <span className="text-[8px] opacity-50 font-mono ml-1">{log.confidence}</span>
                                   </div>
                                 </div>
                                 ...
                                 <div className="md:w-3/4 flex gap-6">
                                   <div className="w-1/3">
                                     ...
                                     <span className="text-[10px] font-mono text-brand-secondary uppercase border border-brand-secondary/30 bg-brand-secondary/10 px-1.5 py-0.5 rounded inline-block">
                                       {log.project}
                                     </span>
                                   </div>
                                   <div className="w-2/3">
                               ...
                               <div className="pt-4 border-t border-white/5 flex items-center justify-end gap-3">
                                 <button className="text-[10px] font-mono uppercase text-white/30 hover:text-white mr-auto transition-colors">
                                   {copy.FEEDBACK_VIEW.ACTIONS.OVERRIDE}
                                 </button>
                                 
                                 <button className="flex items-center gap-2 px-4 py-2 rounded bg-white/5 text-white/50 hover:text-white hover:bg-white/10 transition-colors text-[10px] font-mono uppercase tracking-widest">
                                   <FolderArchive size={12} /> {copy.FEEDBACK_VIEW.ACTIONS.REJECT}
                                 </button>

                                 {log.aiTag === 'SYSTEM BUG' ? (
                                   <button className="flex items-center gap-2 px-4 py-2 rounded bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-white transition-colors text-[10px] font-mono uppercase tracking-widest border border-red-500/30">
                                     <CheckCircle2 size={12} /> {copy.FEEDBACK_VIEW.ACTIONS.VALIDATE_BUG}
                                   </button>
                                 ) : log.aiTag === 'FEATURE REQUEST' ? (
                                   <button className="flex items-center gap-2 px-4 py-2 rounded bg-blue-500/10 text-blue-400 hover:bg-blue-500 hover:text-white transition-colors text-[10px] font-mono uppercase tracking-widest border border-blue-500/30">
                                     <CheckCircle2 size={12} /> {copy.FEEDBACK_VIEW.ACTIONS.VALIDATE_FEATURE}
                                   </button>
                                 ) : (
                                    <button className="flex items-center gap-2 px-4 py-2 rounded bg-brand-primary/10 text-brand-primary hover:bg-brand-primary hover:text-black transition-colors text-[10px] font-mono uppercase tracking-widest border border-brand-primary/30">
                                     <CheckCircle2 size={12} /> DISMISS & ARCHIVE
                                   </button>
                                 )}
                               </div>
       ...
       {/* (Queue Bulks Actions & Invite Modal logic remain unchanged) */}
       {selectedIds.size > 0 && activeTab === 'QUEUE' && (
         <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-bg-app/95 backdrop-blur-2xl border border-brand-primary/30 shadow-[0_0_40px_rgba(6,182,212,0.15)] rounded-full px-2 py-2 flex items-center gap-2 z-40 animate-in slide-in-from-bottom-10 fade-in duration-300">
            ...
            <button 
              ...
              className="flex items-center gap-2 px-6 py-2.5 rounded-full text-[10px] font-mono uppercase tracking-widest text-black bg-brand-primary hover:bg-brand-primary/90 transition-colors group"
            >
       ...
       {isInviteModalOpen && (
         <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-bg-app border border-white/10 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col">
               ...
               <div className="p-6 border-t border-white/5 flex items-center justify-end gap-3 bg-white/2">
                 <button 
                   onClick={() => setIsInviteModalOpen(false)}
                   className="px-6 py-2.5 rounded text-xs font-mono uppercase text-white/50 hover:text-white hover:bg-white/5 transition-colors border border-transparent hover:border-white/10"
                 >
                   {copy.INVITE_MODAL.BTN_CANCEL}
                 </button>
                 <button 
                   onClick={handleAuthorizeDeployment}
                   className="px-6 py-2.5 rounded text-xs font-mono uppercase font-bold text-black bg-brand-primary hover:bg-brand-primary/90 transition-all shadow-[0_0_15px_rgba(6,182,212,0.4)]"
                 >
                   {copy.INVITE_MODAL.BTN_DEPLOY}
                 </button>
               </div>
